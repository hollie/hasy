...
if (new_minute) {
	...
	$sunblinds_enabled = 0;
	if ($Season eq "Summer" and $Weather{TempOutside} >= 20) {
		$sunblinds_enabled = 1;
	}
...
if (new_minute) {
    ...
    if (state $blinds_command eq 'down') {
    	...
    	if (time_greater_than "06:30" and time_greater_than "$Time_Sunrise - 0:10" and time_less_than "10:00" and !$recent) {
    	...
    	# unless we are before "06:30"
    	if ($recent and time_greater_than "06:30" and $lightlevel > 20) {
    		print_log "Blinds up based on lightlevel == $lightlevel";
    		set $blinds_command 'up';
    	}
    ...
    if (state $blinds_command ne 'down') {
    ...
    # Sunscreen mode, to be extended with other parameters
    # For the time being is it only time-based.
    #set $blinds_command 'sun1'   if (time_now "08:00" and $sunblinds_enabled);
    #set $blinds_command 'sun45'  if (time_now "12:00" and $sunblinds_enabled);
    #set $blinds_command 'unsun1' if (time_now "15:30" and $blinds_command->state() eq 'sun45');
  	#set $blinds_command 'up'     if (time_now "17:30");
