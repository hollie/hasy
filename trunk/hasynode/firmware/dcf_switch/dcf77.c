...
// -----------------------------------------------------------------------------------------
// The switchpoints are stored in flash and use a simple dataformat.
// Firstly, there's a counter that keeps track of the number of switch points that are valid.
// That counter is located @ 0x0FFF in program memory.
// In the next code block, the switchpoints are stored. 
// Every point takes 4 program memory locations
...
#pragma DATA 0x0FFC, 0x00, 0x00, 0x00, 0x03
#pragma DATA 0x1000, 0x07, 0x00, 0x7C, 0x01, 0x07, 30, 0x03, 0x01, 22, 0x00, 0x7F, 0x00
...
void print_switch_list(){
	...
	char nr_of_points = flash_read(DATA_START_ADDRESS-1);
	char count = 0;
...
switch_point get_switch_point(char index){
	switch_point point;
	short address = DATA_START_ADDRESS + (index * 4);
	point.hour = flash_read(address);
	if (point.hour == 0xAA){
		return point;
	}
	address++;
	point.minute   = flash_read(address);
	address++;
	point.mask     = flash_read(address);
	address++;
	point.action   = flash_read(address);
	address++;
...
void put_switch_point(switch_point point){
	...
	flash_loadbuffer(point.hour);
	flash_loadbuffer(point.minute);
	flash_loadbuffer(point.mask);
	flash_loadbuffer(point.action);
	short address = point.position * 4 + DATA_START_ADDRESS;
	flash_write(address);
...
void add_switch_point(){
	switch_point point;
	point.position = flash_read(DATA_START_ADDRESS-1);
	...
	flash_loadbuffer(0x00);
	flash_loadbuffer(0x00);
	flash_loadbuffer(0x00);
	flash_loadbuffer(point.position+1);
	flash_write(DATA_START_ADDRESS-0x04);
...
void delete_switch_point(){
	...
	nr_of_points = flash_read(DATA_START_ADDRESS-1);
	...
	flash_loadbuffer(0x00);
	flash_loadbuffer(0x00);
	flash_loadbuffer(nr_of_points-1);
	flash_write(DATA_START_ADDRESS-0x04);
