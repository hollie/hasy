...
;  Global Symbols
;-----------------------------------------------
export  _UART_IN_TX_ISR
export  _UART_IN_RX_ISR

IF (UART_IN_RXBUF_ENABLE)
export  UART_IN_aRxBuffer
export _UART_IN_aRxBuffer
export  UART_IN_bRxCnt
export _UART_IN_bRxCnt
export  UART_IN_fStatus
export _UART_IN_fStatus
ENDIF
...
IF (UART_IN_RXBUF_ENABLE)
 UART_IN_fStatus:
_UART_IN_fStatus:      BLK  1
 UART_IN_bRxCnt:
_UART_IN_bRxCnt:       BLK  1
AREA UART_IN_RAM (RAM, REL, CON)
 UART_IN_aRxBuffer:
_UART_IN_aRxBuffer:    BLK UART_IN_RX_BUFFER_SIZE
ENDIF
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: _UART_IN_TX_ISR
;
;  DESCRIPTION:
;     UART TX interrupt handler for instance UART_IN.
;
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: _UART_IN_RX_ISR
;
;  DESCRIPTION:
;     UART RX interrupt handler for instance UART_IN.
...
IF (UART_IN_RXBUF_ENABLE)
   ...
   mov  X,[UART_IN_bRxCnt]                                 ; Load X with byte counter
   mov  A,REG[UART_IN_RX_CONTROL_REG]                      ; Read the control register
   push A                                                  ; Store copy for later test
                                                           ; IF real RX interrupt
   and  A,UART_IN_RX_REG_FULL                              ; Did really really get an IRQ
...
.UARTRX_ReadRx:
   ...
   or   [UART_IN_fStatus],A                                ; Set error flags (parity,framing,overrun) bits
   tst  REG[UART_IN_RX_BUFFER_REG], 0x00                   ; Read the data buffer to clear it.
   and  A,UART_IN_RX_FRAMING_ERROR                         ; Check for framing error special case
   jz   .RESTORE_IDX_PP                                    ; Not framing error, all done

                                                           ; Disable and re-enable RX to reset after
                                                           ; framing error.
   and   REG[UART_IN_RX_CONTROL_REG], ~UART_IN_RX_ENABLE   ; Disable RX
   or    REG[UART_IN_RX_CONTROL_REG],  UART_IN_RX_ENABLE   ; Enable RX
   jmp  .RESTORE_IDX_PP                                    ; Done with framing error, leave.


.UARTRX_NO_ERROR:
   mov  A,REG[UART_IN_RX_BUFFER_REG ]                      ; Read the data buffer

                                                           ; IF buffer not full
   tst  [UART_IN_fStatus],UART_IN_RX_BUF_CMDTERM           ; Check for buffer full
   jnz  .RESTORE_IDX_PP                                    ; All done

   cmp  A,UART_IN_CMD_TERM                                 ; Check for End of command
   jnz  .UARTRX_CHK_BACKSPACE
   or   [UART_IN_fStatus],UART_IN_RX_BUF_CMDTERM           ; Set command ready bit



   RAM_SETPAGE_IDX >UART_IN_aRxBuffer
   RAM_CHANGE_PAGE_MODE FLAG_PGMODE_10b
   mov  [X + UART_IN_aRxBuffer],00h                        ; Zero out last data
   RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b
   jmp  .RESTORE_IDX_PP

.UARTRX_CHK_BACKSPACE:                                     ; 
IF(UART_IN_BACKSPACE_ENABLE)                               ; Enable if backspace/delete mode
   cmp  A,UART_IN_BACKSPACE_ENABLE                         ; Check for backspace character
   jnz  .UARTRX_IGNORE                                     ; If not, skip the backspace stuff
   cmp  [UART_IN_bRxCnt],00h                               ; Check if buffer empty
   jz   .RESTORE_IDX_PP                                    ; 
   dec  [UART_IN_bRxCnt]                                   ; Decrement buffer count by one.
   jmp  .RESTORE_IDX_PP
ENDIF                                                      ; 

.UARTRX_IGNORE:
IF(UART_IN_RX_IGNORE_BELOW)                                ; Ignore charaters below this value
   cmp  A,UART_IN_RX_IGNORE_BELOW                          ; If ignore char is set to 0x00, do not
   jc   .RESTORE_IDX_PP                                    ; ignore any characters.
ENDIF

.UARTRX_CHK_OVFL:                                          ; Check for MAX String here
	
   RAM_SETPAGE_IDX >UART_IN_aRxBuffer                      ;   using idexed address mode
   cmp  [UART_IN_bRxCnt],(UART_IN_RX_BUFFER_SIZE - 1)
   jc   .UARTRX_ISR_GETDATA
   RAM_CHANGE_PAGE_MODE FLAG_PGMODE_10b
   mov  [X + UART_IN_aRxBuffer],00h                        ; Zero out last data in the buffer
   RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b
   or   [UART_IN_fStatus],UART_IN_RX_BUF_OVERRUN           ; Set error flags (parity,framing,overrun) bits
   jmp  .RESTORE_IDX_PP

.UARTRX_ISR_GETDATA:                                       ; IF input data == "CR", then end of command
                                                           ; X is already loaded with pointer
   RAM_CHANGE_PAGE_MODE FLAG_PGMODE_10b
   mov  [X+UART_IN_aRxBuffer],A                            ; store data in array
   RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b
   inc  X                                                  ; Inc the pointer
   mov  [UART_IN_bRxCnt],X                                 ; Restore the pointer
